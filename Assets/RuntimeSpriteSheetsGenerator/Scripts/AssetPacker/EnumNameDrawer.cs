...
public class DependAttirbute : PropertyAttribute
{
    /// <summary>
    /// 枚举名称
    /// </summary>
    public string name;
    public DependAttirbute(string name)
    {
        this.name = name;
    }
}
...
public class EnumNameDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        bool isElement = System.Text.RegularExpressions.Regex.IsMatch(property.displayName, "Element \\d+");
        if (isElement)
        {
    ...
    private void DrawEnum(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            System.Reflection.FieldInfo info = type.GetField(names[i]);
